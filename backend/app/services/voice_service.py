...
class VoiceService:
    ...
    @staticmethod
    def get_voice_prompt(language: str, key: str) -> Dict[str, Any]:
        from app.services.localization_service import localization_service
        text = localization_service.get_prompt_text(language, key)
        if not text:
            return {
                "success": False,
                "language": language,
                ...
                "error": f"Voice prompt key '{key}' not found for language '{language}'."
            }

        audio_rel_path = f"/static/audio/{language}/{key}.mp3"
        audio_full_path = os.path.join(settings.VOICE_AUDIO_DIR, language, f"{key}.mp3")
        audio_exists = os.path.exists(audio_full_path)
        ...
        return {
            ...
            "audio_url": audio_rel_path if audio_exists else None
        }
